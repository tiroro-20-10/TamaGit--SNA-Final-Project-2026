...
13. [Тесты](#13-тесты)
14. [Docker](#14-docker)
15. [Очистка и повторный прогон](#15-очистка-и-повторный-прогон)
...
`status` сначала пытается взять свежий state с VPS, а если сервер недоступен, показывает локальный cache.
...
`live` читает state напрямую с VPS каждые 5 секунд, если `TAMAGIT_VPS_URL` задан. Если VPS недоступен, он падает на локальный cache.
...
Если issue не создаются автоматически при смерти питомца, смотри `docker compose logs webhook`. Чаще всего причина в том, что у токена нет прав на создание issues (`repo` / `issues:write`), либо в `.env` не задан `GITHUB_TOKEN`.
...
## 15. Очистка и повторный прогон

Чтобы прогнать весь сценарий заново на сервере:

```bash
# Останови контейнеры
cd /opt/TamaGit && docker compose down

# Очисти локальный кэш TamaGit
rm -rf ~/.tamagit

# Если нужен совсем чистый серверный прогон — удали папку проекта
rm -rf /opt/TamaGit
```

После этого можно заново клонировать репозиторий, выполнить `tamagit server-setup` на VPS и `tamagit setup` у участников команды.
